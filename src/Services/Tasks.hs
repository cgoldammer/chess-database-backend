...
data Task = Task { taskName :: String, taskGames :: [Entity Game], taskDB :: String, userName :: Maybe String }
type Tasks = [Task]
...
moveScheduledToActive (AllTasks (first:rest) Nothing done) = AllTasks rest (Just first) done

addTask :: AllTasks -> Task -> AllTasks
addTask (AllTasks sched act done) task = moveScheduledToActive $ AllTasks (task : sched) act done

completeActiveTask :: AllTasks -> AllTasks
completeActiveTask tasks@(AllTasks _ Nothing _) = tasks
completeActiveTask (AllTasks sched (Just t) done) = moveScheduledToActive $ AllTasks sched Nothing (t:done)
